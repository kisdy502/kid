...
public class JavaNioSocketServer {

    public static void main(String[] agrs) {
        JavaNioSocketServer socketServer = new JavaNioSocketServer();
        socketServer.start();
    }
    ...
    public void start() {
        ...
        try (ServerSocketChannel serverSocketChannel = ServerSocketChannel.open()) {
            ...
            //创建消息处理器
            ServerHandler handler = new ServerHandlerImpl(1024);

            while (flag == 1) {
                int i = selector.select();
                System.out.println("开始处理请求:" + i);
                //获取selectionKeys并处理
